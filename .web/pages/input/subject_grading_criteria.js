...
export default function Component() {
  ...
  function getData_gbbfharw([col, row]){
    return formatDataEditorCells(col, row, state.subjec_grade_state.table_col, state.subjec_grade_state.table_data);
  }
  
    const handleSubmituosfmmpi = useCallback((ev) => {
        const $form = ev.target
        ev.preventDefault()
        const form_data = {...Object.fromEntries(new FormData($form).entries()), ...{"평가요소": getRefValue(ref_평가요소), "주제": getRefValue(ref_주제), "목표": getRefValue(ref_목표)}}

        addEvents([Event("state.subjec_grade_state.handle_register", {form_data:form_data})])
  ...
  {`교과 채점 기준을 구성하기 위해 주제, 목표, 평가요소를 입력하세요.`}
</Text>
  <Divider sx={{"marginBottom": "1em", "marginTop": "0.5em"}}/>
</Box>
  <Box as={`form`} onSubmit={handleSubmituosfmmpi}>
  <VStack>
  ...
  <Box sx={{"textAlign": "left", "width": "100%"}}>
  <Button onClick={(_e) => addEvents([Event("state.subjec_grade_state.handle_clear", {})], (_e), {})}>
  {`빈 줄 삭제`}
</Button>
</Box>
  <Box sx={{"width": "100%"}}>
  <div style={{"width": "100%", "height": "100vh"}} sx={{"width": "100%", "height": "100vh"}}>
  <DataEditor columnSelect={`multiple`} columns={state.subjec_grade_state.table_col} data={state.subjec_grade_state.table_data} drawFocusRing={true} getCellContent={getData_gbbfharw} getCellForSelection={true} onCellEdited={(_pos,_data) => addEvents([Event("state.subjec_grade_state.handle_update", {pos:_pos,new_vaule:_data})], (_pos,_data), {})} onPaste={true} rowHeight={40} rowMarkers={`both`} rows={state.subjec_grade_state.table_data.length} smoothScrollX={true} smoothScrollY={true} sx={{"height": "100vh"}}/>
</div>
</Box>
</VStack>
</Container>
