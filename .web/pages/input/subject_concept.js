...
import { Avatar, Box, Button, Center, Container, Divider, Drawer, DrawerContent, DrawerOverlay, Heading, HStack, Image, Link, Menu, MenuButton, MenuDivider, MenuItem, MenuList, Modal, ModalBody, ModalContent, ModalHeader, ModalOverlay, option, Select, Spacer, Text, VStack } from "@chakra-ui/react"
...
export default function Component() {
  ...
  function getData_xhmfnmob([col, row]){
    return formatDataEditorCells(col, row, state.subjec_state.table_col, state.subjec_state.table_data);
  }
  const ref_학년도 = useRef(null); refs['ref_학년도'] = ref_학년도;
  const ref_학년 = useRef(null); refs['ref_학년'] = ref_학년;
  const ref_학기 = useRef(null); refs['ref_학기'] = ref_학기;
  const ref_교과 = useRef(null); refs['ref_교과'] = ref_교과;
  
    const handleSubmitkesfcfga = useCallback((ev) => {
        const $form = ev.target
        ev.preventDefault()
        const form_data = {...Object.fromEntries(new FormData($form).entries()), ...{"성취기준": getRefValue(ref_성취기준), "교과": getRefValue(ref_교과), "단원": getRefValue(ref_단원), "학년도": getRefValue(ref_학년도), "학년": getRefValue(ref_학년), "학기": getRefValue(ref_학기)}}

        addEvents([Event("state.subjec_state.handle_register", {form_data:form_data})])
  ...
  const ref_단원 = useRef(null); refs['ref_단원'] = ref_단원;
  const ref_성취기준 = useRef(null); refs['ref_성취기준'] = ref_성취기준;
  ...
  {`교과 성취 기준을 구성하기 위해 학년도, 학기, 학년, 교과, 단원을 입력하세요.`}
</Text>
  <Divider sx={{"marginBottom": "1em", "marginTop": "0.5em"}}/>
</Box>
  <Box as={`form`} onSubmit={handleSubmitkesfcfga}>
  <VStack>
  <Select id={`학년도`} placeholder={`학년도`} ref={ref_학년도} sx={{"colorSchemes": "twitter"}}>
  <option value={`2022`}>
  {`2022`}
</option>
  <option value={`2023`}>
  ...
  <Select id={`학기`} placeholder={`학기`} ref={ref_학기} sx={{"colorSchemes": "twitter"}}>
  <option value={`1학기`}>
  {`1학기`}
</option>
  <option value={`2학기`}>
  ...
  <Select id={`학년`} placeholder={`학년`} ref={ref_학년} sx={{"colorSchemes": "twitter"}}>
  <option value={`1학년`}>
  {`1학년`}
</option>
  <option value={`2학년`}>
  ...
  <Select id={`교과`} placeholder={`교과`} ref={ref_교과} sx={{"colorSchemes": "twitter"}}>
  <option value={`국어`}>
  {`국어`}
</option>
  <option value={`과학`}>
  ...
  <Select id={`단원`} placeholder={`단원 선택`} ref={ref_단원} sx={{"colorSchemes": "twitter"}}>
  <option value={`지권의 변화`}>
  {`지권의 변화`}
</option>
  <option value={`여러 가지 힘`}>
  {`여러 가지 힘`}
</option>
  <option value={`생물의 다양성`}>
  {`생물의 다양성`}
</option>
</Select>
  <Select id={`성취기준`} placeholder={`성취기준 선택`} ref={ref_성취기준} sx={{"colorSchemes": "twitter"}}>
  <option value={`[9과01-01] 지구계의 구성 요소를 알고, 지권의 층상 구조와 그 특징을 설명할 수 있다.`}>
  {`[9과01-01] 지구계의 구성 요소를 알고, 지권의 층상 구조와 그 특징을 설명할 수 있다.`}
</option>
  <option value={`[9과01-02] 지각을 이루는 암석을 생성 과정에 따라 분류할 수 있으며, 암석의 순환 과정을 설명할 수 있다.`}>
  ...
  <Box sx={{"textAlign": "left", "width": "100%"}}>
  <Button onClick={(_e) => addEvents([Event("state.subjec_state.handle_clear", {})], (_e), {})}>
  {`빈 줄 삭제`}
</Button>
</Box>
  <Box sx={{"width": "100%"}}>
  <div style={{"width": "100%", "height": "100vh"}} sx={{"width": "100%", "height": "100vh"}}>
  <DataEditor columnSelect={`multiple`} columns={state.subjec_state.table_col} data={state.subjec_state.table_data} drawFocusRing={true} freezeColumns={5} getCellContent={getData_xhmfnmob} getCellForSelection={true} onCellEdited={(_pos,_data) => addEvents([Event("state.subjec_state.handle_update", {pos:_pos,new_vaule:_data})], (_pos,_data), {})} onPaste={true} rowHeight={40} rowMarkers={`both`} rows={state.subjec_state.table_data.length} smoothScrollX={true} smoothScrollY={true} sx={{"height": "100vh"}}/>
</div>
</Box>
</VStack>
</Container>
