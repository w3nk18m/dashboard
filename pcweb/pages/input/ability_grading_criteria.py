...
채점기준: List[str] = [
    "학습 기대치 이상의성과를 지속적으로 달성함.",
    "학습 기대치를 충족하여, 때로는 넘어서는 성과를 달성함.",
    "학습 기대치를 충족하기 위해 노력하고 있으며, 지속적으로 성장하고 있음.",
    "이해력 습득 초기 과정 중에 있거나 학습 기대치에 아직 도달하지 못함.",
]

class AbilityGradeState(State):
    table_col: list = []
    table_data: list = []
    table_null: list = []
    ...
    table_col = [
        ...
        { "title": "역량", "type": "str" , "width": 120 },
        { "title": "수준", "type": "str" , "width": 120 },
        { "title": "입력", "type": "str" , "width": 460 },
        { "title": "채점기준", "type": "str" , "width": 660 },
    ]
    table_null = ["","","","","","",""]
...
@webpage(path="/input/ability_grading_criteria")
def ability_grading_criteria():
    return rx.container(
        rx.vstack(
            rx.box(
                rx.heading("역량 채점 기준", font_size=styles.H1_FONT_SIZE, mt=12, mb=4),
                rx.text("역량 채점 기준을 구성하기 위해 주제, 목표, 평가요소를 입력하세요.", color=tc["docs"]["body"]),
                rx.divider(),
                text_align="left",
                width="100%",
            ),
            rx.form(
                rx.vstack(
                    ...
                    rx.input(
                        placeholder="역량", id="역량"
                    ),
                    rx.button("등록", type_="submit"),
                ),
                on_submit=AbilityGradeState.handle_register,
            ),
            rx.box(
                rx.divider(),
            ...
            rx.box(
                rx.button("빈 줄 삭제", on_click=AbilityGradeState.handle_clear),
                text_align="left",
                width="100%",
            ),
            rx.box(
                rx.data_editor(
                    columns=AbilityGradeState.table_col,
                    data=AbilityGradeState.table_data,
                    row_height=40,
                    height="100vh",
