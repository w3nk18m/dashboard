...
def component_grid():
    posts = []
    for path, document in inputs.items():
        meta = document.metadata
        posts.append(
            rx.link(
                rx.box(
...
@webpage(path="/input/index")
def input():
    return rx.container(
        rx.vstack(
            rx.box(
                rx.heading("자료 입력", font_size=styles.H1_FONT_SIZE, mt=12, mb=4),
                rx.text(
